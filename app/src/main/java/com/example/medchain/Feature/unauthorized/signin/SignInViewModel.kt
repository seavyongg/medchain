...
class SignInViewModel @Inject constructor(
    ...
    fun onTokenChanged(token: String): String {
        this.token = token
        var message = ""
        if (token.isEmpty()) message = "Token is required"
        else if (token.length < 10) message = "Token must be at least 10 characters long"
        return message
    }
